    ...
    <div class="md:w-8/12 lg:w-6/12 px-5 bg-white p-6 rounded-lg shadow-xl">
        <form action="{{ route('categories.update', $category->id) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                @error('name') border-red-500  @enderror" value="{{$category->name}}">
                @error('name')
            ...
            <div class="mb-5">
                ...
                @error('description') border-red-500  @enderror" >{{$category->description}}</textarea>
                @error('description')
                    <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-1 text-center">{{$message}}</p>    
                @enderror
            </div>
            <input type="submit" value="Editar categoría" class="bg-sky-600 hover:bg-sky-700 transition-colors cursor-pointer uppercase w-full p-3 text-white rounded-lg">
        </form>
    </div>    
</div>
@endsection